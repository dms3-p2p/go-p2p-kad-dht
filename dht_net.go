...
func (dht *Dms3FsDHT) handleNewStream(s inet.Stream) {
	go dht.handleNewMessage(s)
}

func (dht *Dms3FsDHT) handleNewMessage(s inet.Stream) {
	ctx := dht.Context()
...
func (dht *Dms3FsDHT) sendRequest(ctx context.Context, p peer.ID, pmes *pb.Message) (*pb.Message, error) {
...
func (dht *Dms3FsDHT) sendMessage(ctx context.Context, p peer.ID, pmes *pb.Message) error {
	ms, err := dht.messageSenderForPeer(p)
	if err != nil {
		return err
	}
...
func (dht *Dms3FsDHT) updateFromMessage(ctx context.Context, p peer.ID, mes *pb.Message) error {
...
func (dht *Dms3FsDHT) messageSenderForPeer(p peer.ID) (*messageSender, error) {
	dht.smlk.Lock()
	ms, ok := dht.strmap[p]
	if ok {
		dht.smlk.Unlock()
